...
public class ProblemOccurenceResultCollector implements ISessionResultCollector<InvocationSequenceData, List<ProblemOccurrence>> {
	...
	public List<ProblemOccurrence> collect(SessionContext<InvocationSequenceData> sessionContext) {
		...
		for (Tag leafTag : leafTags) {
			...
			AggregatedDiagnosisInvocationData rootCauseInvocations = getRootCauseInvocations(leafTag);
			CauseStructure causeStructure = getCauseStructure(leafTag);
			RootCause rootCause = new RootCause(rootCauseInvocations.getMethodIdent(), rootCauseInvocations, rootCauseInvocations.getRawInvocationsSequenceElements());

			// create new ProblemOccurrence
			ProblemOccurrence problem = new ProblemOccurrence(inputInvocationSequence, globalContext, problemContext.getCommonContext(), rootCause, causeStructure);
	 ...
	 */
	private AggregatedDiagnosisInvocationData getRootCauseInvocations(Tag leafTag) {
		while (null != leafTag) {
			if (leafTag.getType().equals(RuleConstants.DIAGNOSIS_TAG_PROBLEM_CAUSE)) {
				...
				if (leafTag.getValue() instanceof AggregatedDiagnosisInvocationData) {
					return (AggregatedDiagnosisInvocationData) leafTag.getValue();
				} else {
